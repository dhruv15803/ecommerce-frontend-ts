      ...
      <div className="flex flex-col xl:flex-row gap-4 m-10">
        ...
        <div className="mx-10 flex xl:w-[20%] xl:mx-0 flex-col border-2 rounded-lg shadow-lg p-4">
          ...
          {cart.map((item) => {
            return (
              <div
                className="flex items-center my-1 justify-between"
                key={item.cartid}
              >
                <span>
                  {item.productname} ({item.itemqty}x)
                </span>
          ...
          <button
            onClick={checkOut}
            className="my-4 p-2 border-2 rounded-lg border-red-500 text-red-500 hover:bg-red-500 hover:text-white hover:duration-300"
          >
            Checkout
          </button>
        </div>
